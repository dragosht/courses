		...
		size_t size, loff_t *offset)
{
	...
	if (!tmp) {
		printk(KERN_ALERT "so2_kbd: unable to allocate buffer\n");
		return -ENOMEM;
	}
...
static int so2_kbd_init(void)
{
	...
	/* the following pieces of code will fail, so leave them commented */
	/*
	if (request_region(I8042_DATA_REG, 1, MODULE_NAME) == NULL) {
		// this will always fail, but why?
		// keyboard has exclusive access: cat /proc/ioports!
		err = -EBUSY;
		goto out2;
	}
	...
	*/
	...
	err = request_irq(I8042_KBD_IRQ, so2_kbd_interrupt_handle, IRQF_SHARED,
				MODULE_NAME, &devs[0]);
	if (err != 0) {
		printk(LOG_LEVEL "ERROR: request_irq: error: %d\n", err);
		goto out2;
