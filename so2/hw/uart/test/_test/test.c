...
test1(void)
{
	...
	err |= test("major",
			system("cat /proc/devices | grep '" XSTR(COM1_MAJOR) " " MODULE_NAME "' >/dev/null 2>&1"), 0);
	err |= test("ioports COM1",
			system("cat /proc/ioports | grep '03f8-03ff : " MODULE_NAME "' > /dev/null 2>&1"), 0);
	err |= test("ioports COM2",
			system("cat /proc/ioports | grep '02f8-02ff : " MODULE_NAME "' > /dev/null 2>&1"), 0);
	err |= test("interrupts COM1",
			system("cat /proc/interrupts | grep '4:.*" MODULE_NAME "' > /dev/null 2>&1"), 0);
	err |= test("interrupts COM2",
			system("cat /proc/interrupts | grep '3:.*" MODULE_NAME "' > /dev/null 2>&1"), 0);
	...
	err |= test("major",
			system("cat /proc/devices | grep '" XSTR(COM2_MAJOR) " " MODULE_NAME "' >/dev/null 2>&1"), 0);
	err |= test("rmmod", system("rmmod " MODULE_NAME), 0);
	if (err)
		return err;
	...
	err |= fatal_test("insmod " MODULE_NAME ", COM2 only",
			system("insmod " MODULE_NAME ".ko option=" XSTR(OPTION_COM2_ONLY)), 0);
	err |= fatal_test("insmod " SOLUTION_NAME ", COM1 only",
			system("insmod " SOLUTION_NAME ".ko option=" XSTR(OPTION_COM1_ONLY)), 0);
	err |= test("ioports COM1",
			system("cat /proc/ioports | grep '03f8-03ff : " SOLUTION_NAME "' > /dev/null 2>&1"), 0);
	err |= test("ioports COM2",
			system("cat /proc/ioports | grep '02f8-02ff : " MODULE_NAME "' > /dev/null 2>&1"), 0);
	err |= test("interrupts COM1",
			system("cat /proc/interrupts | grep '4:.*" SOLUTION_NAME "' > /dev/null 2>&1"), 0);
	err |= test("interrupts COM2",
			system("cat /proc/interrupts | grep '3:.*" MODULE_NAME "' > /dev/null 2>&1"), 0);
...
test2(void)
{
	...
	if (fd == -1)
		fail("open " UART0);
#define ioctl_test(n)	test("invalid ioctl " XSTR((n)), ioctl(fd, UART16550_IOCTL_SET_LINE, (n)), -1)
...
#undef ioctl_test
	...
	err |= not_test("insmod " MODULE_NAME ", option=0xdeadbabe",
			system("insmod " MODULE_NAME ".ko option=0xdeadbabe"), 0);
...
static const struct {
	...
	{
		.num = 3,
		.speed = { UART16550_BAUD_1200, UART16550_BAUD_2400, UART16550_BAUD_4800, -1 },
		.bufsizes = { 128, 256 },
	},
	{
		.num = 4,
		.speed = { UART16550_BAUD_9600, UART16550_BAUD_19200, UART16550_BAUD_38400, UART16550_BAUD_56000 },
		.bufsizes = { 256, 1024 },
	},
	{
		.num = 1,
...
{
	int r;
	line->baud = speed_sets[speed_set].speed[rand() % speed_sets[speed_set].num];
...
gen_test_file(char *fname, int speed_set)
{
	...
	sprintf(comm, "dd if=/dev/urandom of=%s bs=1 count=%d >/dev/null 2>/dev/null", fname, size);
	if (system(comm))
		...
		int num_tests)
{
	...
	if (reader != writer) {
		...
		sprintf(cbuf, "insmod %s.ko option=%d", reader, OPTION_COM2_ONLY);
		fatal_test(dbuf, system(cbuf), 0);
		sprintf(dbuf, "insmod %s", writer);
		sprintf(cbuf, "insmod %s.ko option=%d", writer, OPTION_COM1_ONLY);
		fatal_test(dbuf, system(cbuf), 0);
	} else {
		sprintf(dbuf, "insmod %s", reader);
	...
	if (fd1 == -1)
		fail("open " UART1);
	err |= test("ioctl reader", ioctl(fd1, UART16550_IOCTL_SET_LINE, &uli), 0);
	err |= test("ioctl writer", ioctl(fd0, UART16550_IOCTL_SET_LINE, &uli), 0);
