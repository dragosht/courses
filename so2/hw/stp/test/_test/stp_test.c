 ...
 *
 * 2011, Operating Systems 2 - Ixia Challenge
 */
...
static ssize_t sendto_message(int sockfd, struct sockaddr_stp *sas, char *buf, size_t len)
{
	return sendto(sockfd, buf, len, 0, (SSA *) sas, sizeof(*sas));
}

static ssize_t sendmsg_message(int sockfd, struct sockaddr_stp *sas, char *buf, size_t len)
{
	struct iovec iov;
	struct msghdr msg;
...
static void test_sendto(void)
{
	...
	memcpy(remote_sas.sas_addr, ether_aton("00:00:00:00:00:00"), sizeof(remote_sas.sas_addr));
...
static int stp_proc_read_values(void)
{
	...
	/* read values line */
	fscanf(f, "%d %d %d %d %d %d", &rx_pkts, &hdr_err, &csum_err, &no_sock, &no_buffs, &tx_pkts);
...
int wait_for_semaphore(sem_t *sem, unsigned secs)
{
	struct timespec ts;
	int ret;
...
static int run_sender_receiver(enum socket_action action)
{
	...
	/* Create two named semaphores used to communicate with the child processes */
	sem_r = sem_open(SEM_NAME_RECEIVER, O_CREAT);
	assert(sem_r != SEM_FAILED);
	sem_s = sem_open(SEM_NAME_SENDER, O_CREAT);
