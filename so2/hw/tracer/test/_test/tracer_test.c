...
static void init_test(void)
{
	...
	system("mknod /dev/tracer c 10 42");
}
...
static void init_helper(int *fd)
{
	...
	system("mknod /dev/helper c 10 30");
...
static void test_dev_minor_major(void)
{
	...
	test(__FUNCTION__, major(buf.st_rdev) == 10 && minor(buf.st_rdev) == 42, 1);
...
int tracer_proc_check_values(struct tracer_stats *st, struct test_case *tc, int no)
{
	...
	for (idx = 0; idx < no; idx++) {
		...
		for (idk = 0; idk < no; idk++) {
			...
			for (idz = 0; idz < tc[idx].test_params.kcalls; idz++) {
				...
				total_free += tc[idx].test_params.free[idz] * tc[idx].test_params.alloc[idz];
			...
			dprintf("tr_alloc (%d): got %d, expected %d\n", st[idk].tr_pid, st[idk].tr_alloc, tc[idx].test_params.kcalls);
...
static void tracer_proc_read_values(struct tracer_stats *st, int no)
{
	...
	for (i = 0; i < no; i++) {
		fscanf(f, "%d %d %d %d %d %d %d %d %d %d", &st[i].tr_pid, &st[i].tr_alloc, &st[i].tr_free,
		       &st[i].tr_mem, &st[i].tr_mem_free, &st[i].tr_sched, &st[i].tr_up, &st[i].tr_down, &st[i].tr_lock, &st[i].tr_unlock);
	}
	fclose(f);
}
